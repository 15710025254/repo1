...
public class InsEquipmentCateService : SqlSugarRepository<InsEquipmentCate>, IInsEquipmentCateService
{
    ...
    //public async Task<List<SysUser>> GetInsEquipmentCateList()
    //{
    //    //var provider = _db.AsTenant().GetConnectionScope("DormitoryInfo");
    //    //var aa = provider.Queryable<SysUser>();
    //    //var bb = await _repository.GetAllAsync();
    ...
    {
        List<InsEquipmentCate> list = await _repository.GetListAsync(x => x.Num, OrderByType.Asc);
        return list;
    }
